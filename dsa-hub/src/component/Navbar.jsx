...
const Navbar = ({user}) => {
  ...
  <div class="relative flex max-w-screen-xl flex-col overflow-hidden px-4 py-4 md:mx-auto md:flex-row md:items-center">
    <Link to='/dashboard' class="flex items-center whitespace-nowrap text-2xl font-black">
      ...
      <span class="text-black">DSA Hub</span>
    </Link>
    ...
    <nav aria-label="Header Navigation" class="peer-checked:mt-8 peer-checked:max-h-56 flex max-h-0 w-full flex-col items-center justify-between overflow-hidden transition-all md:ml-24 md:max-h-full md:flex-row md:items-start">
      <ul class="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
        <li class="text-gray-600 md:mr-12 hover:text-blue-600"><Link to='/dashboard'>Dashboard</Link></li>
        <li class="text-gray-600 md:mr-12 hover:text-blue-600"><Link to='/topics'>Topicwise Questions</Link></li>
        <li class="text-gray-600 md:mr-12 hover:text-blue-600"><Link to='/profile'>Profile</Link></li>
        <li class="text-gray-600 md:mr-12 hover:text-blue-600"><Link to='/resource'>Resource</Link></li>
        <li class="text-gray-600 md:mr-12 hover:text-blue-600">
          <button onClick={signOutUser} class="rounded-md border-2 border-blue-600 px-6 py-1 font-medium text-blue-600 transition-colors hover:bg-blue-600 hover:text-white">Log Out</button>
        </li>
      </ul>
    </nav>
  </div>
